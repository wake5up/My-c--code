...
#include <ctemplate/template.h>//第三方库
#include <string>
#include <vector>
...
class OjView
{
    public:
        ...
        static void ExpandAllQuestionshtml(std::string* html, std::vector<Question>& ques)
        {
            ...
            ctemplate::TemplateDictionary dict("all_questions");//数据字典

            for(const auto& que:ques)
            {
                ctemplate::TemplateDictionary* section_dict = dict.AddSectionDictionary("question");//子字典--各个题目
